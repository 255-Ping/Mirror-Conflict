...
signal door_triggered(door_id: String)
...
func _key_detection():
	...
	if Input.is_action_just_pressed("switch_cameras"):
		if camera.is_current():
			other_player.camera.make_current()
			other_player.ui.visible = true
			ui.visible = false
		else:
			camera.make_current()
			other_player.ui.visible = false
			ui.visible = true
...
func _on_push_box_area_entered(area: Area2D) -> void:
